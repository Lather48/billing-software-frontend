...
const BillsList = () => {
    ...
    const fetchBills = async () => {
        try {
            let url = `https://billing-software-backend-production-0456.up.railway.app/api/bills?`;
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to cancel and delete this bill? Stock will be reverted.')) {
            try {
                await axios.delete(`https://billing-software-backend-production-0456.up.railway.app/api/bills/${id}`);
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4 items-end">
                ...
                <div className="w-full md:w-auto flex gap-2">
                    <div>
                        ...
                        <input
                            type="date"
                            className="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-primary sm:text-sm"
                ...
                {(searchTerm || statusFilter || dateRange.start || dateRange.end) && (
                    <button
                        onClick={() => { setSearchTerm(''); setStatusFilter(''); setDateRange({ start: '', end: '' }) }}
