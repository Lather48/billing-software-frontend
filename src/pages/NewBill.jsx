...
const NewBill = () => {
        ...
        const fetchData = async () => {
            try {
                const [custRes, prodRes] = await Promise.all([
                    axios.get('https://billing-software-backend-production-0456.up.railway.app/api/customers'),
                    axios.get('https://billing-software-backend-production-0456.up.railway.app/api/products')
                ]);
    ...
    const handleSaveBill = async (action = 'save', skipPaymentCheck = false) => {
        ...
        try {
            const res = await axios.post('https://billing-software-backend-production-0456.up.railway.app/api/bills', billData);
    ...
    const filteredProducts = products.filter(p => p.name.toLowerCase().includes(searchProduct.toLowerCase()) || p.code.toLowerCase().includes(searchProduct.toLowerCase()));
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select
                                    className="mt-1 w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:ring-primary sm:text-sm"
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                        ...
                        <div className="relative mb-4">
                            <input
                                ...
                                className="w-full border border-gray-300 rounded-md py-2 px-3 pl-10 focus:outline-none focus:ring-primary sm:text-sm"
                            />
                            {searchProduct && (
                                        ...
                                        totals.enrichedItems.map((item, index) => (
                                            <tr key={index}>
                                                ...
                                                <td className="px-3 py-2">
                                                    <input type="number" min="1" value={item.qty} onChange={(e) => updateItem(index, 'qty', Number(e.target.value))} className="w-full border border-gray-300 rounded px-1 py-1 text-sm focus:ring-primary focus:border-primary" />
                                                </td>
                                                <td className="px-3 py-2">
                                                    <input type="number" min="0" value={item.price} onChange={(e) => updateItem(index, 'price', Number(e.target.value))} className="w-full border border-gray-300 rounded px-1 py-1 text-sm focus:ring-primary focus:border-primary" />
                                                </td>
                                                <td className="px-3 py-2">
                                                    <input type="number" min="0" max="100" value={item.discount} onChange={(e) => updateItem(index, 'discount', Number(e.target.value))} className="w-full border border-gray-300 rounded px-1 py-1 text-sm focus:ring-primary" />
                                                </td>
                                                ...
                                                <td className="px-3 py-2 text-right">
                                                    <button onClick={() => removeItem(index)} className="text-danger hover:text-red-700 p-1"><FiTrash2 size={16} /></button>
                                                </td>
                                            </tr>
                                        ))
                                    )}
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 sticky top-6">
                        ...
                        <div className="space-y-3 text-sm">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span className="font-medium text-gray-900">₹{totals.taxableAmount.toFixed(2)}</span>
                            </div>
                            <div className="flex justify-between text-gray-500 text-xs">
                                <span>CGST:</span>
                                <span>₹{totals.cgst.toFixed(2)}</span>
                            </div>
                            <div className="flex justify-between text-gray-500 text-xs pb-3 border-b">
                                <span>SGST:</span>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {['cash', 'upi', 'card', 'credit'].map(mode => (
                                    <button
                                        ...
                                        className={`px-3 py-2 rounded text-sm font-medium capitalize border transition-all ${paymentMode === mode ? 'bg-primary text-white border-primary' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}
                                    >
                                        {mode}
                                    </button>
                                ))}
                        ...
                        {paymentMode !== 'credit' && (
                            <div className="mt-4">
                                ...
                                <input
                                    ...
                                    placeholder={`Amount eq. to ₹${totals.grandTotal}`}
                                    className="mt-1 w-full border border-gray-300 rounded-md py-2 px-3 text-lg font-medium focus:outline-none focus:ring-primary"
                                />
                                {paidVal > totals.grandTotal && (
                                    <p className="mt-2 text-sm text-green-600 font-medium">Change Due: ₹{(paidVal - totals.grandTotal).toLocaleString()}</p>
                                )}
                                {paidVal < totals.grandTotal && paidVal > 0 && (
                                    <p className="mt-2 text-sm text-amber-600 font-medium">Balance Due: ₹{(totals.grandTotal - paidVal).toLocaleString()}</p>
                                )}
                            </div>
                        )}
                        ...
                        <div className="space-y-3">
                            <button
                                onClick={() => {
                                    ...
                                    handleSaveBill(pendingAction, true);
                                }}
                            ...
                            <button
                                ...
                                className="w-full bg-white hover:bg-gray-50 text-gray-600 border border-gray-300 font-medium py-2 px-4 rounded-lg transition"
                            >
                                Cancel
                            </button>
                        </div>
